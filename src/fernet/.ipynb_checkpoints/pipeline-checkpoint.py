...
def gradient_descent(dwis, bvals, bvecs, mask, init_f, init_tensor, niters=50): 
    ...
    for i in range(niters):
        if optimal_num is not None: break
        iternum = i+1
        iternum_list.append(iternum)
        print(" - Iteration {0:d} out of {1:d}.".format(iternum, niters))
        
        grad1, predicted_signal_tissue, predicted_signal_water = \
            grad_data_fit_tensor(lower_triangular, signal, H, bvals, 
                                 volume_fraction)
        grad1_nona = grad1[np.logical_not(np.isnan(grad1))]
        grad1_avg = np.mean(np.abs(grad1_nona))
        grad1_avg_list.append(grad1_avg)
        print("\tgrad1 avg: {0:0.4e}".format(grad1_avg))
        ...
        prediction_error_nona = prediction_error[np.logical_not(np.isnan(prediction_error))]
        prederr_avg = np.mean(prediction_error_nona)
        prederr_avg_list.append(prederr_avg)

        # STEVE: if second order derivative of average error is approaching 0, record that
        if any(np.isclose(np.diff(prederr_avg_list, 2), 0)):
            if optimal_num is None:
                optimal_num = iternum

        print("\tpred error avg: {0:0.4e}".format(prederr_avg))
        
        gradf = (bvals * (predicted_signal - signal) \
                           * (predicted_signal_tissue - predicted_signal_water)).sum(-1)
        gradf_nona = gradf[np.logical_not(np.isnan(gradf))]
        gradf_avg = np.mean(np.abs(gradf_nona))
        gradf_avg_list.append(gradf_avg)
        print("\tgradf avg: {0:0.4e}".format(gradf_avg))
        volume_fraction -= weight * step_size * gradf

        # STEVE: pretty sure I added that to fix an error that it gives due to NaN in array
    ...
    gd_info = {
        'iternum': iternum_list,
        'grad1_avg': grad1_avg_list,
        'prederr_avg': prederr_avg_list,
        'gradf_avg': gradf_avg_list,
        'optimal_iterations': optimal_num
    }
    
    # gd_info commented out
    return final_f, final_tensor, gd_info

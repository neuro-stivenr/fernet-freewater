...
def run_fernet(dwis_filename, bvals_filename, bvecs_filename, mask_filename, output_basename,
    ...
    final_f, final_tensor, gd_info = gradient_descent(dwis, bvals, bvecs, mask, 
    ...
    # STEVE: adding pickle outputs for gradient descent info
    with open("{0}_gd_info.pkl".format(output_basename), 'wb') as handle:
        pickle.dump(gd_info, handle)
